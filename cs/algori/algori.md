...
ST表存储: **从每个位置开始 长度为$2^k$的区间答案**

花费O(nlogn)时间建表 花费O(1)时间**查询**

比如 原数组 `arr = [2,3,5,1,4,6,7,0]`
#### 线段树
线段树是用来 **维护区间信息** 的 **二叉树**.
...
树状数组是线段树解决问题的 **子集**, 但是在一些问题中 树状数组得到的**常数因子会更小**.

我们总能将一段前缀 [1, n] 拆成 不多于 $\boldsymbol{\log n}$ 段区间，使得这 log⁡𝑛 段区间的信息是 **已知的**．
