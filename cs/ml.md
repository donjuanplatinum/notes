...
## 交叉验证
交叉验证用于 在训练数据集中每次取出一部分不训练 作为测试数据集 然后评估模型在测试数据集的效果 

最后当测试数据集取遍训练集时 对总结果进行评估


## 混淆矩阵
混淆矩阵是一种**评价分类模型性能**的工具 能直观的展现模型的正确与否

将正确值作为行 预测值作为列. 则**只有对角线上的元素是模型预测正确的**

关于混淆矩阵 有两个指标: 敏感性和特异性
...
- 敏感性: 在 **真的正类** 中 被模型成功预测出来的比例

$$
Sensitivity = \frac{TP}{TP+FN}
$$

- 特异性: 在 **真的负类** 中 被模型正确排除的比例

$$
Sepcificity = \frac{TN}{TN+FP}
$$
### ROC与AUC
ROC曲线描述的是 当改变分类的阈值时 模型 **在正确预测正类和错误预测负类之间的权衡**

ROC的横坐标为 假正率(FPR) 纵坐标为真正率(TPR)也就是敏感性
...
AUC就是ROC曲线下的面积 用于比较不同的训练模型之间的差距
## 回归和分类
回归和分类是机器学习的基本任务
...
可以进行**在线学习** 当模型预测错误时,更新参数 适合处理**流数据**
### Ridge回归
就是 `线性回归+L2正则化`
...
Lasso能估计稀疏的分布 **能精确的设置系数为0** 可以置为0是Ridge回归所不具备的
### ElasticNet
融合了L1和L2正则化 在Lasso与Ridge之间权衡 继承了Ridge在旋转下的**稳定性**

$$
\min_w\frac{1}{2n_{samples}}\|\|Xw -y\|\|_2^2 + \alpha \rho\|w\|_1 + \frac{\alpha(1-\rho)}{2}\|w\|_2^2
$$
### LARS
最小角回归 用于求解线性回归,Lasso回归等

LARS是一个步进算法 **逐步添加变量来构建模型 并根据残差的方向逐步逼近最优解**
...
通过最小角度的方式 将最相关的特征进入模型

在选定的特征方向前进 更新系数

$$
\hat{beta}' = \hat{beta} + \lambda sign(c_j)
$$
### 桥回归
Lasso是加入L1正则化的线性回归  岭回归是加入L2正则化的线性回归

而桥回归介于两者之间 加入Lq范数的正则化的线性回归

$$
\min_w \|Xw -y \|_2^2 + \alpha \sum_{j=1}^{p} |\beta_j|^q ; (0 < q \leq 2)
$$
### 逻辑回归
逻辑回归是一个**分类**模型 是用sigmoid曲线来拟合概率
...
对于贝叶斯公式可以查看statistics.md

对于类别C 输入为X

$$
P(C|X_1,X_2,...,X_n) = P(C) \frac{\prod_{i=1}^{n}P(X_i \mid C)}{P(X_1,X_2,...,X_n)}
$$

在实际运用中 分母$P(X_1,X_2,...,X_n)$是一样的 所以只需要比较$P(C) \prod_{i=1}^{n}P(X_i \mid C)$的大小
#### 成立前提
朴素贝叶斯假设每个特征**相互独立**

#### 分类
其中
- P(C)是**训练数据中类别C出现的概率** 比如在垃圾邮件分类中就是垃圾邮件的出现概率
- 根据训练数据集计算每个特征的条件概率$P(X_i \mid C)$ 比如在垃圾邮件分类中,计算正确邮件对每个词的概率和错误邮件对每个词的概率 
- 分别计算输入的邮件中 假设为正确邮件的先验下和错误的先验下的概率
- 比较使用正确先验和错误先验的概率大小 大的是分类

#### 变体
朴素贝叶斯的变体一般是**似然的分布**的不同

比如高斯朴素贝叶斯是假设 每一个特征的似然分布$P(X_i \mid C)$服从高斯分布
#### 例子
假设我有15个邮件 其中有8个好邮件 7个垃圾邮件 设事件C为这个邮件是好的

那么$P(C) = \frac{8}{15}$

我们计算好邮件条件下每个词的概率和坏邮件条件下每个词的概率 比如

$$
P("Hello"|C) P("Hello"|\tilde{C})....
$$

然后我们在新邮件中分别计算好先验和坏先验假设下的概率

$$
P = P(C) P("nihao"|C) P("monday"|C)...
$$

$$
\tilde{P} = P(\tilde{C}) P("nihao"|\tilde{C}) P("monday"|\tilde{C})...
$$

比较$P$和$\tilde{P}$即为分类
### KNN
KNN可以做回归 也可以分类 属于惰性学习

基本思想是: 看目标对象的附近的K个样本 根据它们的类别来定义目标对象的类别

其中K为超参数

KNN的步骤是 先定义距离方法,然后计算距离,再找到最近的k个邻居,然后使用决策规则

#### 距离
首先需要计算目标对象和其他样本的距离度量
...
#### 决策
在找到最近的k个邻居后 我们对邻居使用决策规则

##### 分类
找到最近的k个近邻的多数属于什么分类 然后判定目标就是这个分类
##### 回归
回归是对最近的k个近邻取**均值**
...
一种经典的**贪婪算法** 广泛用于 压缩感知 从少量测量中 重建稀疏信号

OMP和LARS的区别是
>OMP通过残差和稀疏度的约束来限定更新方向 而LARS通过最相关的特征方向来更新方向

OMP的算法有两种等价形式

这是个类似 LARS的形式 在这个形式中 可以通过设置非0约束w的数量上限来**固定稀疏度**

$$
\arg\min_w \| y - Xw \|_2^2 受约束于 \|w\|_0 \leq n_{nonzero_coefs}
$$

其中
- $\|w\|_0$: 代表w向量中**非零元素**的个数
- $n_{nonzero_coefs}$: 想使用的最多的非零元素的个数

这是另一种形式 目的是**固定误差** 限制误差<= $\epsilon$

$$
\arg\min_w \|w\|_0 受约束于 \|y - Xw\|_2^2 \leq \epsilon
$$
...
传统的回归得到的是估计$\hat{y}$值 而贝叶斯得到的是**参数完整的概率分布**
...
在贝叶斯回归中 输出y被假设为符合**高斯分布**

$$
p(y \mid X,w,\alpha) = N(y \mid Xw,\alpha^{-1})
$$
...
## 聚类
无监督学习算法 在没有标签的情况下 按照相似性对样本分类

### K-Means
K聚类 把样本分为K组 找到这K组的中心
...
1. 随机初始化K个中心$\mu_1,..,\mu_k$
2. 为每个样本找到最近的类别中心$c_j = \arg\min_i \|x_j - \mu_i \|$
3. 对每个类别 计算新的中心 $\mu_i = \frac{1}{|C_i|}\sum_{x \in C_i}x$
## PCA
主成分分析 

**找到数据中方差最大的正交方向(主成分) 然后进行降维**

首先我们进行**中心化数据** 相当于把数据的中心放到坐标轴原点 方便进行其他操作
...
### SVD简化
求协方差矩阵是一个繁杂的过程 我们可以通过SVD(奇异分解) 参见linear.md

我们把第一步 中心化后的数据$\tilde{X}$ 做SVD

$$
\tilde{X} = U \Sigma V^T
$$

其中V就是右奇异向量 也就是PCA的**主成分方向**
...
$$
L = Loss(data,model) + \lambda \Omega(\theta)
$$
...
$$
\hat{w}_{MAP} = \arg\min_w [-log P(D \mid w) - log P(w)]
$$

其中
- -logP(D|w)就是数据的拟合项(损失函数)
- -logP(w)就是正则项

所以加入了正则化就符合了这个式子 如果将高斯先验带入-logP(w) 得到的形式与L2正则化的形式成正比
### L1正则化
L1正则化使用**L1范数**
...
L1范数具有稀疏性 这是由拉普拉斯分布带来的 相比而言 L2范数更加平滑

#### 概率视角
L1正则化思想是 假设参数服从拉普拉斯先验


### L2正则化
L2正则化是防止模型**过拟合**的方法 使用**L2范数**

本质是: 惩罚模型参数过大 让模型的训练更加保守

即 在原**损失函数**后面 加上一项 **参数平方和的惩罚项**

在实现上 可以在**损失函数** 也可以在**优化器**(仅SGD) 添加L2正则化
...
$$
L_{total}(\theta) = L(\theta) + \frac{\lambda}{2} \| \theta \|_2^2
$$
...
L1在强相关特征不稳定 而L2不稀疏 两者的优点结合即为Elastic

$$
\Omega(\theta) = \alpha \| w\|_1 + (1 - \alpha) \| w\|_2^2
$$
...
$$
\theta_{t+1} = \theta{t} - \eta(\mu v_{t+1} + \nabla_{\theta}L(\theta_{t}))
$$

### ASGD
平均随机梯度下降 是SGD的优化版本 但是不支持动量

ASGD会维护一个平均的参数

$$
\tilde{\theta_{t}} = \frac{1}{t - t_0 + 1}\sum_{i=t_0}^{t}\theta_i
$$
...
$$
\eta_{t} = \frac{\eta}{\sqrt{G_t} + \epsilon}
$$

然后进行参数更新

$$
\theta_{t+1} = \theta_t - \frac{\eta_t} g_t
$$

$\epsilon$可以防止当$\sqrt{G_t}$过小时除0

但是我们不难发现

当$G_t -> \infty$时 $\eta_t ->0$ 后续会引起**学习率过小停止学习**

所以Adadelta,RMSprop,Adam横空出世
### RMSProp
适合非平稳目标(如RNN)

是Adagrad的改进

不像Adagrad会累计全部历史梯度,RMSProp只关注**最近一段时间**的梯度大小,同时仍然是每个参数有各自的学习率
...
方向用动量平滑
### AdamW
AdamW修正了Adam的L2正则化的问题

目前非常推荐在 Transformer 和 CNN 中使用 若追求极值的泛化能力还是建议使用SGD

AdamW把L2正则的部分给单独提出来作为一个**中间步骤**

AdamW只有参数更新部分和Adam公式不一样

$$
\theta_{t} = \theta_{t-1} - \eta \cdot (\frac{\hat{m}_t}{\sqrt{\hat{v}_t} + \epsilon} +\lambda \cdot \theta_{t-1})
$$


### AMSGrad
修正了AdamW和Adam的二阶矩估计 保证了收敛性 提供了**严格**的收敛证明

AMSGrad解决了在某些凸优化中可能不收敛的问题 对稀疏的梯度任务更稳定

AMSGrad的$g_t$,$m_t$,$v_t$与Adam和AdamW完全相同
...
- 位置矫正(仅对一阶矩$\hat{m}_t$ AMSGrad不矫正二阶矩$\hat{v}_t$)


### LBFGS
一个优雅的优化器 使用的是**二阶方法** 计算的是几何的二阶曲率 和传统的优化器非常不同


$\boxed{使用历史梯度变化 近似Heissian的逆 然后走牛顿方向}$

不像Adam调整学习率 LBFGS学习的是目标函数的曲率
...
- 超大数据
- 强噪声梯度
- 从头训练CNN/Transformer等
### Muon
Muon算法中 梯度方向比梯度的大小更重要

所以在**模型更新的方向纠缠 参数维度之间相关**时非常好用

利用牛顿–舒尔茨迭代高效地逼近梯度矩阵的正交化形式，并将其用于更新神经网络中的二维权重参数，从而加速训练收敛

Muon的算法是
## 学习率调度器
学习率调度器可以动态的调整学习率和动量 让还没接近最优时收敛更快 接近时收敛更稳
...
细化流程
```
输入(长度为n) -> 分为(batch_size ,seq_len) -> 一个batch一个batch的进入Transformer
```

所以说 Transformer的长期依赖几乎不能跨batch 因为注意力不会跨batch

改进的模型有

1. Transformer-XL: 在训练时缓存上一个段的hidden states以跨batch依赖
...
我们来观察**任意**带有结合律的**二元算子**

我们先从累加开始 加法是符合结合律的二元算子

假设累加 $a_1 + a_2 + a_3 + .. + a_n$
...
我们令它们为$b_1 .. b_n$ 然后相隔2步的加

$$
(b_1 + b_3) (b_2 + b_4) .. (b_{n-2} + b_n)
$$

然后再相隔4步 8步 ...

最终假设在并行线程足够的情况下 我们只需要log_2 n次运算
### 使用
```
...
## Unet
U-Net 是 1995 年提出的医学图像分割经典网络 是`图像分割`领域的标准架构 目前也被运用于很多`扩散模型` 正在被探索用于语言模型.

U-Net 是一种全CNN模型,专门为 `像素级预测任务` 设计,在`小样本`情况表现非常好.

U-Net的核心创新点在于引入了`跳跃连接` 跳远连接可以在上采样的过程中补回空间信息
...
在解码阶段 跳跃连接把编码器中相同分辨率的特征图拼接回来 补回`空间细节`
## 贝叶斯优化
...
PINN不仅能学习到数据样本的分布 还能学习到数学方程描述的物理定律

传统的神经网络是纯数据驱动的 而PINN在训练过程中加入了**物理约束** 可以用更少的数据样本学习更具泛化能力的模型
## ONN
算子神经网络

算子学习是一类深度学习模型 结合了**传统神经网络的学习能力**和**数学算子**

算子神经网络的基本思想是 将神经网络中某些运算(卷积,池化)等 
## MoE
专家混合模型

MoE通过**门控网络**根据输入特征 动态的只激活一小部分专家来参与计算 从而实现了**增大模型容量而计算成本基本不变**

![MoE](../resource/MoE.png)
...
- 门控机制: 通过门控机制选择k个专家 其他专家保持静默

### 流程
1. 计算专家得分

首先输入向量x与一个可学习的路由矩阵$W_g$相乘 然后加入一些噪声(防止某些专家始终不被选中)

$$
H(x) = x \cdot W_g + Noise
$$
